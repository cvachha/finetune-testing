...
def train(args, train_dataset, model, tokenizer):
    ...
    loss_log_file = os.path.join(args.output_dir, f"loss_curve_rank_{rank_id}_task2a.txt")
    with open(loss_log_file, 'w') as f:
        f.write("step,current_loss,avg_loss,time_per_iter_avg,iter_time,eta_minutes\n")
    
    # Setup profiler to skip 1 step and profile 3 steps
    ...
    trace_file = os.path.join(args.output_dir, f"task2a_trace_rank_{rank_id}.json")
    ...
    with torch.profiler.profile(
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if step < 5:
                    print(f"[Rank {rank_id}] minibatch {step + 1} loss: {loss.item():.6f}")
                    ...
                    for param in model.parameters():
                        if param.grad is not None:
                            ...
                            if args.local_rank == 0:
                                gather_params_list = [torch.zeros_like(param.grad) for _ in range(args.world_size)]
                            else:
                                gather_params_list = None
                            
                            torch.distributed.gather(param.grad, gather_params_list, dst=0)
                            

                            # now scatter the grads
                            if args.local_rank == 0:
                                calc_avg_grad = sum(gather_params_list) / args.world_size
                                scatter_avg_list = [calc_avg_grad for _ in range(args.world_size)]
                            else:
                                scatter_avg_list = None
                            
                            output_grad = torch.zeros_like(param.grad)
                            torch.distributed.scatter(output_grad, scatter_avg_list, src=0)
                            
                            param.grad = output_grad
                ...
                # Step the profiler after each batch iteration
                prof.step()
            ...
            evaluate(args, model, tokenizer, prefix="Results for epoch " + str(_))
            ##################################################
...
def main():
    ...
    parser.add_argument("--world_size", type=int, default=1,
                        help="Total number of processes (nodes) participating in distributed training")
    parser.add_argument("--master_ip", type=str, default="10.10.1.2",
                        help="IP address of the master node (use 10.10.1.* for CloudLab experimental network)")
    parser.add_argument("--master_port", type=str, default="12345",
                        help="Port for distributed communication (must be > 1023)")
